...
import com.yf.exam.ability.ai.AIExamGenerationService;
...
public class PaperServiceImpl extends ServiceImpl<PaperMapper, Paper> implements PaperService {
    ...
    public ExamDetailRespDTO paperDetail(String paperId) {
        ...
        List<PaperQuDTO> gapFillingList = new ArrayList<>();
        for(PaperQuDTO item: list){
            ...
            if(QuType.GAP_FILLING.equals(item.getQuType())){
                gapFillingList.add(item);
            }
        }
        ...
        respDTO.setGapFillingList(gapFillingList);
        return respDTO;
    }
    ...
    private List<PaperQu> generateByRepo(String examId){
        return generateByRepo(examId, false);
    }

    /**
     * 题库组题方式产生题目列表（支持AI智能选择）
     * @param examId
     * @param useAI 是否使用AI智能选择
     * @return
     */
    private List<PaperQu> generateByRepo(String examId, boolean useAI){
    ...
    private PaperQu processPaperQu(ExamRepoDTO repo, Qu qu) {
        ...
        if (QuType.SAQ.equals(qu.getQuType())) {
            ExamRepoExtDTO extRepo = (ExamRepoExtDTO) repo;
            if (extRepo.getSaqScore() != null) {
                paperQu.setScore(extRepo.getSaqScore());
                paperQu.setActualScore(extRepo.getSaqScore());
            } else {
                // 默认分值，如果没有配置
                paperQu.setScore(10);
                paperQu.setActualScore(10);
            }
        }

        if (QuType.GAP_FILLING.equals(qu.getQuType())) {
            ExamRepoExtDTO extRepo = (ExamRepoExtDTO) repo;
            if (extRepo.getGapFillingScore() != null) {
                paperQu.setScore(extRepo.getGapFillingScore());
                paperQu.setActualScore(extRepo.getGapFillingScore());
            } else {
                // 默认分值，如果没有配置
                paperQu.setScore(5);
                paperQu.setActualScore(5);
            }
        }
